...
uint8_t POLY_Bytes[39*27] = { 0x50, 0x50, 0x50, 0x0C };								//1053 packets belong to POLY
...
void Packetizer(uint16_t source_ID, uint8_t bytes_Read) {
	...
	case 2:															// POLY
		...
		while (poly_Count < 27) {									// while packets to send
			uint8_t ii = 6;

			POLY_Bytes[0 + poly_Count*39] = 0x50;				//0 39 78 117
			POLY_Bytes[1 + poly_Count*39] = 0x50;				//1 40 79 118
			POLY_Bytes[2 + poly_Count*39] = 0x50;				//2 41 80 119
			POLY_Bytes[3 + poly_Count*39] = 0x0C;				//3 42 81 120
			...
			POLY_Bytes[4 + poly_Count*39] = g_source_ID_Exp_Count >> 8;				// put values into array
			POLY_Bytes[5 + poly_Count*39] = g_source_ID_Exp_Count;

			while (!is_Buffer_Empty()) {							// reads from Circular Buffer into packetized buffer (while not empty is too small
				POLY_Bytes[ii + poly_Count*39] = read_Buffer();
				ii++;
			}

			while (ii < 39) {
				POLY_Bytes[ii + poly_Count*39] = 0x00;								// fill the rest of the packetzied buffer with zeroes
				ii++;
			}
			break;
		}
...
uint8_t get_IMU_Data(uint8_t index) {							// Grabs data from the Science Boards
